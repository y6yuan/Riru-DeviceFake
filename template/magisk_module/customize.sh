...
if [ "$MAGISK_VER_CODE" -ge 21000 ]; then
  CURRENT_MODPATH=$(magisk --path)/.magisk/modules/riru_device_fake
else
  CURRENT_MODPATH=/sbin/.magisk/modules/riru_device_fake
fi
...
if [ ! -d "$MODPATH"/config ] && [ -d "/data/adb/riru/modules/device_fake/config" ]; then
  ui_print "- Use configuration from old versions"
  cp -r "/data/adb/riru/modules/device_fake/config" "$MODPATH"/config
fi
